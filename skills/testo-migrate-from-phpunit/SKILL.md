...
| `#[DoesNotPerformAssertions]` / `$this->expectNotToPerformAssertions()` | `#[ExpectNoAssertions]` from **`Testo\Assert`**, on class, method, or function — no method-call form. It's a two-way contract: a marked test that *does* assert (including `Expect::exception`) is `Status::Risky`, not `Passed`. Keep it only on tests that truly assert nothing. |
